...
// predefines for the normal operation
var client;
var errorState = true; // set error state initially
var errorStateError;
...
module.exports = function(options) {
  ...
  if(client.hasOwnProperty('on')){
    client.on('error', function (err){
      // we are swallowing here, to return with the callback from get/set/et.al.
      errorState = true;
      errorStateError = err;
    });
    client.on('ready', function(){
      errorState = false;
      errorStateError = undefined;
    });
  }
  else {
    errorState = true;
    errorStateError = new Error('MultiCache unable to create redis client');
  }

  return {
    set: function (key, value, ttl, callback) {
      if(errorState){
        return callback(errorStateError, null)
      }
